...
# from BP.ArchitectureSearch.main import save_path_to_codeepneat
...
class Individual:
    ...
    def __init__(self, blueprint:Blueprint, compiler=None, birth=None, model=None, name=None, save_path = None):
        ...
        self._save_path = os.path.join(save_path, 'images')
    ...
    def generate(self, save_fig=True, generation=""):
        ...
        if (save_fig):
            ...
            plt.xlim(l-5,r+5)
            try:
                plt.savefig(f'{self._save_path}/gen{generation}_blueprint_{self.blueprint.mark}_module_level_graph_parent1id{self.blueprint.parents[0].mark}_parent2id{self.blueprint.parents[1].mark}.png', show_shapes=True, show_layer_names=True)
            except:
                plt.savefig(f"{self._save_path}/gen{generation}_blueprint_{self.blueprint.mark}_module_level_graph.png", format="PNG", bbox_inches="tight")
            plt.clf()
        ...
        if (save_fig):
            ...
            plt.xlim(l-5,r+5)
            try:
                plt.savefig(f'{self._save_path}/gen{generation}_blueprint_{self.blueprint.mark}_component_level_graph_parent1id{self.blueprint.parents[0].mark}_parent2id{self.blueprint.parents[1].mark}.png', show_shapes=True, show_layer_names=True)
            except:
                plt.savefig(f"{self._save_path}/gen{generation}_blueprint_{self.blueprint.mark}_component_level_graph.png", format="PNG", bbox_inches="tight")
            plt.clf()
        ...
        for component_id in node_order:
            ...
            # layer_map[component_id] = layer
                ...
                input_from_predecessor = layer_map[list(component_graph.predecessors(component_id))[0]][-1]

                logging.log(21, f"{component_id}: Processing single input from {input_from_predecessor.name}. Current layer is {component.keras_component.name}.")

                if isinstance(component.keras_component, keras.layers.Dense) and len(input_from_predecessor.shape) > 2:
                    logging.info(f"Adding a Flatten() layer between {input_from_predecessor.name} and {component.keras_component.name}")
                    input_to_current_layer = keras.layers.Flatten()(input_from_predecessor)
                else:
                    input_to_current_layer = input_from_predecessor # No flattening needed

                layer = [component.keras_component(input_to_current_layer)]
                logging.log(21, f"{component_id}: Added main layer {layer[0].name}")
                if component.complementary_component != None:
                    layer.append(component.keras_complementary_component(layer[0]))
                    logging.log(21, f"{component_id}: Added complement layer {layer[-1].name}")

            elif component_graph.in_degree(component_id) == 2:
                inputs_to_merge = []
                for predecessor_id in component_graph.predecessors(component_id):
                    input_from_predecessor = layer_map[predecessor_id][-1]

                    logging.log(21, f"{component_id}: Processing multi-input. From {input_from_predecessor.name}. Current layer is {component.keras_component.name}.")

                    if isinstance(component.keras_component, keras.layers.Dense) and len(input_from_predecessor.shape) > 2:
                        logging.info(f"Adding a Flatten() layer for merge branch from {input_from_predecessor.name}")
                        flattened_input = keras.layers.Flatten()(input_from_predecessor)
                        inputs_to_merge.append(flattened_input)
                    else:
                        inputs_to_merge.append(input_from_predecessor)

                logging.info(f"Concatenating {len(inputs_to_merge)} inputs for {component.keras_component.name}")
                merge_layer = keras.layers.concatenate(inputs_to_merge)
                logging.log(21, f"{component_id}: Created merge layer {merge_layer.name}")
        ...
        try:
            ...
            try: # Try chack if dot returns code -6
    ...
    def fit(self, input_x, input_y, training_epochs=1, validation_split=0.15, current_generation="", custom_fit_args=None):
        ...
        print(f"Fitting one individual for {training_epochs} epochs")
      #  if current_generation == 'final':
          #  input('crashing')
...
class Population:
    ...
    """
    def __init__(self, datasets=None, individuals=[], blueprints=[], modules=[], hyperparameters=[], input_shape=None, population_size=1, compiler=None, save_directory: str = None):
    ...
    def create_module_population(self, size, global_configs, possible_components, possible_complementary_components):
        ...
        for n in range(size):
            ...
            new_module = GraphOperator(save_path=self.save_directory).random_module(global_configs, 
                                                        possible_components, 
    ...
    def create_blueprint_population(self, size, global_configs, possible_components, possible_complementary_components,
        ...
        for n in range(size):
            ...
            new_blueprint = GraphOperator(self.save_directory).random_blueprint(global_configs,
                                                            possible_components, 
                                                            possible_complementary_components, 
                                                            input_configs,
                                                            possible_inputs,
  ...
  #  keras.metrics.Recall(name='recall'),
      #  keras.metrics.F1Score(name='f1_score')         
            ...
            new_individual = Individual(blueprint=new_blueprint, name=self.historical_marker.mark_individual(), compiler=compiler, save_path=self.save_directory)
    ...
    def mutate_modules(self, percent, elitism_rate, possible_components, possible_complementary_components):
        ...
        generator_function = GraphOperator(self.save_directory).random_component

        mutation_variants = [GraphOperator(self.save_directory).mutate_by_node_removal,
                             GraphOperator(self.save_directory).mutate_by_node_addition_in_edges,
                             GraphOperator(self.save_directory).mutate_by_node_addition_outside_edges,
                             GraphOperator(self.save_directory).mutate_by_node_addition_in_edges,
                             GraphOperator(self.save_directory).mutate_by_node_replacement]
    ...
    def iterate_generations(self, generations=1, training_epochs=1, validation_split=0.15, mutation_rate=0.5, crossover_rate=0.2, elitism_rate=0.1, possible_components=None, possible_complementary_components=None):
        ...
        iterations = []
        best_scores = []
        csv_file = os.path.join(self.save_directory,'iterations.csv')
        csv_history = open(csv_file, "w", newline="")
        ...
        for generation in range(generations):
            ...
            csv_file = os.path.join(self.save_directory,'iterations.csv')
            with open(csv_file, "a", newline="") as csv_history:
            ...
            try:
                save_path_model = os.path.join(self.save_directory,f'models',f"best_generation_{generation}.h5")
                self.return_individual(best_fitting[0]).model.save(save_path_model)
            except:
    ...
    def train_full_model(self, individual : Individual, training_epochs, validation_split, custom_fit_args=None):
        ...
        # HACK: changed custom fit args to none instead of custom fit args

        history = individual.fit(input_x, input_y, training_epochs, validation_split, current_generation="final", custom_fit_args=None)
        ...
        models_dir = os.path.join(self.save_directory, "models")
        os.makedirs(models_dir, exist_ok=True)

        model_filename = f"full_model_indiv{individual.name}_blueprint{individual.blueprint.mark}.h5"
        full_model_path = os.path.join(self.save_directory, "models", model_filename)
...
class GraphOperator:
    ...
    @staticmethod
    def save_graph_plot(full_file_path, graph):
        plt.subplot(121)
        ...
        plt.savefig(f"{full_file_path}", format="PNG", bbox_inches="tight")
    ...
    def random_module(self, global_configs, possible_components, possible_complementary_components, name=0, layer_type=ModuleComposition.INTERMED):
        ...
        save_dir = f"{self.save_path}/images/"

        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)
        self.save_graph_plot(os.path.join(save_dir, f"module_{name}_{self.count}_module_internal_graph.png"), graph)
        self.count+=1
    ...
    def random_blueprint(self, global_configs, possible_components, possible_complementary_components,
        ...
        self.save_graph_plot(f"{self.save_path}/images/blueprint_{name}_module_level_graph.png", graph)
